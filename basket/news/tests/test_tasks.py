...
@patch('basket.news.utils.look_for_user', autospec=True)
...
class UpdateUserTests(TestCase):
    def _patch_tasks(self, attr, **kwargs):
        patcher = patch('basket.news.tasks.' + attr, **kwargs)
...
class AddSMSUserTests(TestCase):
    ...
    def test_success_with_optin(self):
        ...
        number.
        """
        with patch('basket.news.tasks.ExactTargetDataExt') as ExactTargetDataExt:
